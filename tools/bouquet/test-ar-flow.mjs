...
try {
  ...
  if (hasNestedPage && !hasRootPage) {
    await check('Elsewhere React entry still renders independently at /', async () => {
      const test = await newPage()
      try {
        await test.page.goto(origin + '/')
        await test.page.waitForFunction(() => document.querySelector('#root')?.children.length > 0)
        assert.match(await test.page.locator('#root').innerText(), /Elsewhere/)
        assert.equal(await test.page.locator('#root').isVisible(), true)
        assert.deepEqual(test.errors, [])
      } finally { await test.context.close() }
    })
  }
