...
### Gift-card styling, title, and share image — 2026-09-20 (Codex desktop)

Task / owner / branch: keep the first-screen AR instructions distinct but restrained, and use “To: Omaris, From: Mike :)” on the card and its link-preview thumbnail / coordinating Codex desktop session with independent visual QA / `claude/ar-flower-bouquet-omaris-p6x7vd`. Replaced the filled invitation card and bright button with a thin divider, whitespace, lighter serif heading, soft supporting text, and an outlined plum button. The heading now reads “To: Omaris” with “From: Mike :)” beneath it; custom sender/recipient query values still work and an empty sender hides that line. Both flower HTML pages include exact-title Open Graph/Twitter metadata and the generated 1200×630 `social-card.png`; the image itself carries the title. Added the reproducible `render-social-card.mjs` tool and README instructions. Elsewhere's metadata points to `https://elsewhere.quest/Omaris/`; the standalone copy retains production's currently usable HTTP origin pending its certificate.

Validation actually performed: inspected production screenshots at 320×568 and 430×860 with iOS Quick Look capability emulation. The complete AR instructions and button remain visible without scrolling; no horizontal overflow, overlapping grid rows, or page errors. The button is 46 px tall at both sizes. Computed text contrast is conservatively at least 7.29:1 for the button and about 4.50:1 for supporting copy against the darkest page-gradient stop, with a lighter background behind the upper instructions. Verified the default document/OG title exactly, the custom title/sender for `?to=Julián&from=Alex`, and hidden sender plus `To: Ana` title for `?to=Ana&from=`. After synchronization, root and standalone builds passed; all four built flower HTML pages contain the correct scoped image URLs, exact OG/Twitter title and dimensions. Both built PNGs match the approved 197,213-byte source image (SHA-256 `d99ec03bf54c21605719123ce9ed633b5f182b42ccd2650c59cbc5a7b8b76ef9`). Source copies match production; GLB/USDZ assets are unchanged and were not rebaked. `git diff --check` passes. No new tests were written for this cosmetic/text update.

Delivery / next step: production commit `786ae79db49640794502b6747e9630b67e40dd7c` was pushed and deployed successfully in Pages run `35536431444`. The served landing HTML and 197,213-byte PNG match the build, with HTTP 200 and image/png for the thumbnail. This commit synchronizes the reviewed changes to the existing PR #4 branch; PR #4 remains draft and unmerged. Native iPhone camera placement and the custom-domain HTTPS limitations remain as recorded above; no sharing service's cached thumbnail was claimed refreshed by these metadata checks.
